...
class EtatClass Extends Objet
{
    ...
    public function createReport($param,$filter){
        return $this->getApiJson("/createReport&param={$this->formatString($param)}&filter={$this->formatString($filter)}");
    }
    ...
    public function ech_client($centre, $datedeb, $datefin,$clientdebut,$clientfin,$type_reg,$facCompta,$typeTiers)
    {
        $query = "SELECT *
FROM(
SELECT DO_Tiers,D.CT_NumPayeur,CASE WHEN DO_Tiers IS NULL THEN D.CT_NumPayeur ELSE DO_Tiers END Tiers,A.DO_Domaine,A.DO_Type,A.DO_Piece,DL_MontantTTC,RG_Montant,RC_Montant,ISNULL(DR_Regle,0) DR_Regle
                    FROM F_DOCENTETE A
                    ...
                    AND     (($facCompta=0 AND (   ($typeTiers =0 AND DO_Type in (6,7)) OR ($typeTiers =1 AND DO_Type in (16,17)) ) 
                                ...
                                GROUP BY DO_Domaine,DO_Type,DO_Piece
                    )B ON A.DO_Domaine=B.DO_Domaine AND A.DO_Type=B.DO_Type AND A.DO_Piece=B.DO_Piece
                    LEFT JOIN F_DOCREGL C ON C.DO_Domaine = A.DO_Domaine AND C.DO_Type = A.DO_Type AND C.DO_Piece = A.DO_Piece
                    FULL OUTER JOIN (	SELECT DO_Domaine,DO_Type,DO_Piece,MAX(DR_No)DR_No,MAX(CT_NumPayeur) CT_NumPayeur,MAX(RG_Montant)RG_Montant,SUM(RC_Montant) RC_Montant
                                        FROM F_CREGLEMENT A 
                                        LEFT JOIN F_REGLECH B ON A.RG_No = B.RG_No
                                        WHERE RG_Type IN (0,1)
                                        AND (($typeTiers =0 AND DO_Domaine = 0) OR ($typeTiers =1 AND DO_Domaine = 1))
                    AND     (($facCompta=0 AND (   ($typeTiers =0 AND DO_Type in (6,7)) OR ($typeTiers =1 AND DO_Type in (16,17)) ) 
                                OR ($facCompta=1 AND ($typeTiers =0 AND DO_Type in (6)) OR ($typeTiers =1 AND DO_Type in (16)) ) 
                                OR ($facCompta=2 AND ($typeTiers =0 AND DO_Type in (7)) OR ($typeTiers =1 AND DO_Type in (17)) ))) 
                                        GROUP BY DO_Domaine,DO_Type,DO_Piece) D ON D.DO_Domaine = A.DO_Domaine AND D.DO_Type = A.DO_Type AND D.DO_Piece = A.DO_Piece AND C.DR_No = D.DR_No)A
                    WHERE ('-1'= $type_reg OR ISNULL(DR_Regle,0) = '$type_reg')
                    AND         ('0'='$clientdebut' OR Tiers>='$clientdebut') 
    ...
    public function etatCommande($de_no,$ar_refdebut,$ar_reffin,$famille){
        $query ="   DECLARE @DE_No AS INT
                    ...
                    AND ('0'=@AR_RefDebut OR a.AR_Ref>=@AR_RefDebut)
                    AND ('0'=@AR_RefFin OR a.AR_Ref<=@AR_RefFin)
                    AND ('0'=@FA_CodeFamille OR FA_CodeFamille=@FA_CodeFamille)
                    AND AS_QteMini<>0
...
FROM(SELECT *,CASE WHEN (DO_Type=4 OR DO_Type=14) THEN -CAHTBrut ELSE CAHTBrut END Rem
    ...
    public function etatDette($depot_no, $datedeb, $datefin,$client,$ct_num){
        return"SELECT		$client L.CT_NUM,CT_Intitule,ISNULL(SUM(RC_Montant),0)RC_Montant, SUM(DL_MontantTTC)DL_MontantTTC,ISNULL(SUM(RC_Montant),0)-SUM(DL_MontantTTC) AS MONTANT  
                FROM            (SELECT DO_Domaine,DO_Type,DO_Piece,CT_Num,DE_No,DO_Date, SUM(DL_MontantTTC) DL_MontantTTC 
                            	FROM F_DOCLIGNE 
                                GROUP BY DO_Domaine,DO_Type,DO_Piece,CT_Num,DE_No,DO_Date) L
                INNER JOIN 	".$this->db->baseCompta.".DBO.F_COMPTET C
                ON		L.CT_NUM = C.CT_Num
                INNER JOIN	F_DOCREGL D ON L.DO_Piece=D.DO_Piece AND DR_Regle = 0
                ...
                WHERE		L.DO_Type = 6
                AND		l.DO_Date >= '$datedeb' AND l.DO_Date<= '$datefin' 
                AND		($depot_no='0' OR DE.DE_No= $depot_no) 
                AND		('$ct_num'='' OR L.CT_Num= '$ct_num') 
                GROUP BY	$client L.CT_NUM,CT_Intitule 
                ORDER BY	CT_Intitule";
    }

    public function stat_clientParAgence($depot, $datedeb, $datefin,$do_type,$id=0) {
        $requete="  SELECT  CT_Intitule,
		TotNbDoc,
 ...
 AND fArt.AR_HorsStat = 0
		...
		GROUP BY NumTiers  
                ...
                INNER JOIN ".$this->db->baseCompta.".DBO.F_COMPTET C ON C.CT_Num = NumTiers
        ...
        return"
                ...
                FROM (
                    ...
                    SELECT  CASE WHEN RG_TypeReg = 2 THEN 0 ELSE RG_No END RG_No,RG_Date,RG_Libelle,R_Intitule,N_Reglement,CASE WHEN RG_TypeReg = 2 THEN 0 ELSE RG_Type END RG_Type,RG_TypeReg
                            ...
                            ,CASE WHEN (RG_Type = 0 AND RG_Montant<0) OR (RG_BANQUE =3 AND RG_TypeReg=0) OR (RG_BANQUE <>3 AND (RG_Type=1 OR RG_TypeReg = 4))  OR RG_TypeReg = 3 THEN ABS(RG_Montant) ELSE 0 END AS DEBIT 
                            ,CASE WHEN RG_Type = 3 OR (RG_Type = 0 AND RG_Montant>=0) OR (RG_BANQUE =3 AND RG_TypeReg=4) OR RG_TypeReg = 5 THEN ABS(RG_MONTANT) ELSE 0 END AS CREDIT
                            ,CT_Intitule
                ...
                LEFT JOIN (SELECT RG_No,SUM(RC_Montant) RC_Montant
                 ...
                 LEFT JOIN P_REGLEMENT R ON R.cbIndice = C.N_Reglement
                LEFT JOIN F_COMPTET T ON T.CT_Num = C.CT_NumPayeur
...
BEGIN 
     SELECT *,CASE WHEN DL_Qte<0 THEN 1 ELSE 0 END Sens INTO #TMP
                    ...
                    FROM F_DOCENTETE E
                    INNER JOIN F_DOCLIGNE D ON D.DO_Domaine = E.DO_Domaine AND D.DO_Type = E.DO_Type AND E.DO_Piece = D.DO_Piece
...
,CASE WHEN CT_Intitule IS NULL THEN DT.DE_Intitule ELSE CT_Intitule END CT_Intitule,CASE WHEN DL_MvtStock=3 THEN -1 ELSE 1 END * ABS(DL_Qte),D.DO_Type,D.DO_Domaine,DL_MvtStock,AR_PrixAch,DL_PrixRU,CASE WHEN DL_MvtStock=3 THEN -(DL_PrixRU*ABS(DL_Qte)) ELSE (DL_PrixRU*ABS(DL_Qte)) END cumul
                    ...
                    INNER JOIN F_DOCLIGNE D ON D.DO_Domaine = E.DO_Domaine AND D.DO_Type = E.DO_Type AND E.DO_Piece = D.DO_Piece
                    INNER JOIN F_ARTICLE AR ON AR.AR_Ref=D.AR_Ref
                    LEFT JOIN F_COMPTET T ON T.CT_Num = D.CT_Num
                    INNER JOIN F_DEPOT DE ON DE.DE_No=D.DE_No
                    LEFT JOIN (SELECT CAST(DE_No AS VARCHAR(50)) DE_No,DE_Intitule FROM F_DEPOT) DT ON DT.DE_No = D.CT_Num
                    WHERE (@depot ='0' OR D.DE_No = @depot) 
                    AND DL_MvtStock<>0
                    AND ('0' = @articleDebut OR AR.AR_Ref>=@articleDebut)
                    AND ('0' =@articleFin OR AR.AR_Ref<=@articleFin)
...
END";
    ...
    public function etatVrstBancaire($ca_no, $datedeb, $datefin,$type){
        return "SELECT Lien_Fichier,RG_Piece,CT_NumPayeur,RG_Libelle,RG_Montant,ISNULL(RC_Montant,0)RC_Montant,RG_Montant-ISNULL(RC_Montant,0) ResteAPayer,CA_Intitule,CO_Nom,
                CAST(RG_Date AS DATE) RG_Date,RG_BANQUE,CASE WHEN RG_Type=2 THEN 'Non lettré' ELSE 'lettré'END Type_lettre,RG_TypeReg,RG_Type,N_Reglement
                FROM F_CREGLEMENT C
                ...
                LEFT JOIN DBO.F_COLLABORATEUR Co ON Co.CO_No=C.CO_NoCaissier
