...
//    $rows = $result->fetchAll(PDO::FETCH_OBJ);
    ...
    foreach($rows as $row){
        ...
        $result=$objet->db->requete($objet->stat_articleParAgence($val, $datedeb, $datefin,$famille,$articledebut,$articlefin,$do_type,$_SESSION["id"]));
        $rows = $result->fetchAll(PDO::FETCH_OBJ);
        $i=0;
        $canetht=0;
        $canetttc=0;
...
<table class="facture">
    <tr style="text-align: center">
            ...
            <?php if($flagPxRevient==0) echo "<th style='width:50px'>% de Marge</th>"; ?>
    ...
    <?php
        if($rows==null){
            ...
            foreach ($rows as $row){
                ...
                if($flagPxRevient==0) echo "<td style='width:50px;text-align:right'>".ROUND($row->PourcMargeHT,2)."</td>";
                echo"<td style='width:50px;text-align:center'>".$objet->formatChiffre(ROUND($row->TotQteVendues,2))."</td>";
                if($flagPxRevient==0) { echo "<td style='width:70px;text-align:right'>".$objet->formatChiffre(ROUND($row->TotPrxRevientU,2))."</td>";}
                echo "</tr>";
        ...
        echo "<td style='font-size:12px'>Total</td><td></td><td style='font-size:12px;text-align:right'>".$objet->formatChiffre($canetht)."</td><td style='font-size:12px;text-align:right'>".$objet->formatChiffre($precompte)."</td><td style='font-size:12px;text-align:right'>".$objet->formatChiffre($canetttc)."</td>";
...
<table>
    <tr style='background-color:#a4a4a4;font-weight: bold'>
        ...
        <td style="padding:10px"><?php echo $objet->formatChiffre(ROUND($totalMargeG/$totalCANetHTG*100,2)); ?></td>
        <?php } ?>
    </tr>
</table>
<?php
