...
<form action="indexMVC.php?module=9&action=1" method="GET">
    ...
    <thead>
        <tr>
            ...
            <?php if($protected!=1){ ?><td style="float:right"><a href="indexMVC.php?module=9&action=2"><button type="button" id="nouveau" class="btn btn-primary">Nouveau</button></a></td> <?php } ?>
        </tr>
        <tr>
            <td>
...
<table id="table" class="table">
    ...
    <tbody id="liste_planComptable">
        <?php
        $objet = new ObjetCollector();   
        $result=$objet->db->requete($objet->getPlanComptable($val));     
        $rows = $result->fetchAll(PDO::FETCH_OBJ);
        $i=0;
        $classe="";
        if($rows==null){
            ...
            echo "<tr class='article $classe' id='compte_".$row->CG_Num."'>"
                    . "<td><a href='indexMVC.php?module=9&action=2&CG_Num=".$row->CG_Num."'>".$row->CG_Num."</a></td>"
                    . "<td>".$row->CG_Intitule."</td>"
                    . "<td><a href='Traitement\Structure\Comptabilite\PlanComptable.php?CG_Num=".$row->CG_Num."&acte=suppr' onclick=\"if(window.confirm('Voulez-vous vraiment supprimer ".$row->CG_Num."?')){return true;}else{return false;}\"><i class='fa fa-trash-o'></i></a></td>";
                    echo "</tr>";
            }
        }
      ?>
