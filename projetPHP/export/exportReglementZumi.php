...
$etat = new EtatClass();
$rows = $etat->getReglementByClient2($client, $caisse, $type, $treglement, $datedeb, $datefin, 0, $collab, $typeSelectRegl);
$total = 0;
foreach ($rows as $row)
...
<?php
        ...
        foreach ($rows as $row){
            ...
            echo "<tr>
                    ...
                    <td style='text-align:left;width: 50px'>{$row->RG_Date}</td><td style='text-align:left'>{$row->RG_Libelle}</td>
