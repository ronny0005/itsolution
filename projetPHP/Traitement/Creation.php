        ...
        if ($rows != null) {
            foreach ($rows as $row) {
                ...
                if (preg_match('#^[\w.-]+@[\w.-]+\.[a-z]{2,6}$#i', $email)) {
                    $mail = new Mail();
                    $mail->sendMail($msgMail."<br/><br/><br/> {$this->db->db}", $email,  $titreMail);
                }
            }
        }
    }
...
if(strcmp($_GET["acte"],"creation_conditionnement") == 0){
    ...
    if ($rows == null) {
        ...
        if ($artClient->cbMarq== "") {
            $article->insertFArtClient($_GET["AR_Ref"],$_GET["nbCat"]);
        }
...
if(strcmp($_GET["acte"],"maj_prix_detail") == 0){
    ...
    $artClient = new ArtClientClass($AR_Ref,$ncat,$article->db);
    if($artClient->cbMarq=="")
