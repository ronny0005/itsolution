...
if(strcmp($_GET["acte"],"ajout") == 0){
    ...
    $result = $objet->db->requete($objet->insertFComptetC($JO_Num,$JM_Date,$EC_Jour,$EC_Date,$EC_Piece,$EC_RefPiece,$EC_TresoPiece,$CG_Num,$CG_NumCont,$CT_Num,$EC_Intitule,$N_Reglement,$EC_Echeance,$EC_Sens,$EC_Montant,$CT_NumCont,$TA_Code,$EC_Reference,0,0,"",0));
...
function afficheLigne($JO_Num,$mois,$annee){
    ...
    foreach ($rows as $row){
        ...
        if($row->EC_Echeance_C=="1900-01-01") echo ""; else echo $row->EC_Echeance_C; echo "</td>
                <td id='tabEC_MontantDebit'>{$objet->formatChiffre($row->EC_MontantDebit,2)}</td>
                <td id='tabEC_MontantCredit'>{$objet->formatChiffre($row->EC_MontantCredit,2)}</td>
