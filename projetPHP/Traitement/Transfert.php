...
if($_GET["acte"] =="ajout_ligne"|| $_GET["acte"] =="modif"){
    ...
    if($_GET["quantite"]!=""){
        ...
        if (isset($_GET["PROT_No"])) {
            ...
            $isVisu = $docEntete->isVisu($protection->PROT_Administrator, $protection->protectedType("Transfert"), $protection->PROT_APRES_IMPRESSION,$isSecurite);
            if (!$isVisu) {
...
if($_GET["acte"] =="suppr"){
    ...
    if (isset($_GET["PROT_No"])) {
        ...
        if (!$isVisu) {
            ...
            $article = new ArticleClass($AR_Ref, $objet->db);
            $docligneSec->delete($_GET["PROT_No"]);
