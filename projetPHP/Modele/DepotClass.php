...
class DepotClass Extends Objet{
    ...
    public function setCatTarif(){
        ...
        if(sizeof($rows)==0)
            ...
            $this->CA_CatTarif = $rows[0]->CA_CatTarif;
    }
    ...
    public function majCatTarif(){
        $query = "BEGIN 
                    ...
                    IF NOT EXISTS(SELECT 1 FROM Z_DEPOT_DETAIL WHERE DE_No ={$this->DE_No}) 
                        INSERT INTO Z_DEPOT_DETAIL 
							VALUES({$this->DE_No},{$this->CA_CatTarif})
                    ELSE 
                        UPDATE 	Z_DEPOT_DETAIL 
							SET CA_CatTarif = {$this->CA_CatTarif} 
						WHERE 	DE_No={$this->DE_No}
                  END";
        $this->db->query($query);
    }
