...
class ArticleClass Extends Objet{
    ...
    {
        parent::__construct($this->table, $id, 'cbAR_Ref',$db);
    ...
    public function insertFArtClient($ncat){
        ...
        $artClient->insertFArtClient($this->AR_Ref,$ncat,$this->AR_PrixTTC);
    }
    ...
    public function isArticleLigne(){
        $query= "SELECT TOP 1 Ligne FROM F_DOCLIGNE WHERE cbAR_Ref='{$this->AR_Ref}'";
        $result = $this->db->query($query);
        return $result->fetchAll(PDO::FETCH_OBJ);
    }
...
";
    ...
    public function updateArtStock($de_no, $qte, $montant,$protNo=0,$action="")
    {
        ...
        $query = "BEGIN 
                      SET NOCOUNT ON;
                IF EXISTS (SELECT 1 FROM F_ARTSTOCK WHERE DE_No=$de_no AND AR_Ref='{$this->AR_Ref}')
                UPDATE F_ARTSTOCK SET AS_QteSto=AS_QteSto+$qte,cbModification=GETDATE(),cbCreateur=".$_SESSION["id"].",
                ...
                ELSE
                    INSERT INTO F_ARTSTOCK  
                        VALUES(/*AR_Ref*/'{$this->AR_Ref}',/*DE_No*/$de_no
                               ...
                               ,/*DP_NoControle*/0,/*cbDP_NoControle*/NULL
                               ,/*AS_QteAControler*/0,/*cbProt*/0
                               ,/*cbCreateur*/".$_SESSION["id"].",/*cbModification*/GETDATE()
                               ,/*cbReplication*/0,/*cbFlag*/0);
                END;";
