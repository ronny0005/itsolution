...
class ReglementClass Extends Objet{
    ...
    {
        parent::__construct($this->table, $id, 'RG_No',$db);
    ...
    public function getMajAnalytique($dateDeb,$dateFin,$statut,$caNum=''){
        $query = "SELECT A.RG_No,RG_Libelle,RG_Montant,ca.CA_Intitule,EC_No,N_Analytique,Cpa.CA_Num,RG_Montant AS EA_Montant,0 as EA_Quantite
                    FROM F_CREGLEMENT A
    ...
    public function setMajAnalytique($datedeb,$datefin,$caNum){
        $query = "INSERT INTO F_ECRITUREA (EC_No,N_Analytique,EA_Ligne,CA_Num,EA_Montant,EA_Quantite,cbCreateur,cbModification)
                    SELECT	EC_No,N_Analytique,1,Cpa.CA_Num,RG_Montant,0,{$this->userName},GETDATE()
                    FROM	F_CREGLEMENT A
                    INNER JOIN F_CAISSE CA ON CA.CA_No = A.CA_No
                    INNER JOIN Z_RGLT_COMPTEA B ON A.RG_No = B.RG_No
                    INNER JOIN F_COMPTEA Cpa ON Cpa.CA_Num = B.CA_Num
...
END;";
    ...
    public function deleteF_ReglementVrstBancaire($rgNo){
        $requete = " DELETE FROM F_CREGLEMENT
                     WHERE RG_No=(SELECT RG_NoCache FROM [dbo].[Z_RGLT_VRSTBANCAIRE] WHERE RG_No=$rgNo);
                     DELETE FROM [dbo].[Z_RGLT_VRSTBANCAIRE] WHERE RG_No=$rgNo";
        $this->db->query($requete);
    }
    ...
    public function insertF_Reglement(){
        ...
        $requete = "BEGIN 
                ...
                INSERT INTO [dbo].[F_CREGLEMENT] 
                 ...
                 VALUES 
                    ...
                    (CASE WHEN '{$this->CT_NumPayeur}' ='' THEN '' 
               ...
               ,/*RG_Souche*/{$this->RG_Souche},/*CT_NumPayeurOrig*/(CASE WHEN '{$this->CT_NumPayeurOrig}' ='' THEN '' 
        ...
        $rows = $result->fetchAll(PDO::FETCH_OBJ);
        $creglement = new ReglementClass($rows[0]->RG_No);
        $log = new LogFile();
    ...
    public function getReglementByClient($ct_num,$ca_no,$type,$treglement,$datedeb,$datefin,$caissier,$collab,$typeSelectRegl=0) {
        $query= "   
                    ...
                    SET @ctNum = '$ct_num';
                    SET @collab = $collab;
                    SET @nReglement = $treglement;
                    SET @caNo = $ca_no;
                    SET @coNoCaissier = $caissier; 
                    SET @rgType = $typeSelectRegl;
                    DECLARE @admin INT
                    DECLARE @ProtNo INT
                    SET @ProtNo = {$_SESSION["id"]}
...
WITH _ReglementCredit_ AS (
	...
	LEFT JOIN (SELECT DO_Domaine,DO_Type,cbDO_Piece,RG_No
				FROM F_REGLECH
...
, _LettrageCredit_ AS (
	...
	FROM (
		...
		FROM    F_ECRITUREC A
		WHERE   EC_Lettre=1
		AND		CT_Num = (SELECT CT_Num FROM _ReglementCredit_ )
		AND		CG_Num = (SELECT CG_Num FROM _ReglementCredit_ ) 
		AND     CAST(DATEADD(DAY,A.EC_Jour-1,A.JM_Date) AS DATE) = (SELECT RG_Date FROM _ReglementCredit_) 
	) A
)
...
SELECT	
		...
		,EC_Sens = CASE WHEN DO_Domaine IS NOT NULL THEN
						...
						ELSE 0 END 
					ELSE 
						CASE WHEN @pLigneNeg = 0 THEN 1 ELSE 0 END 
					END
...
, _Lettrage_ AS (
	...
	FROM (
		...
		FROM    F_ECRITUREC A
		WHERE   EC_Lettre=1
		AND		CT_Num = (SELECT CT_Num FROM _Reglement_)
		AND		CG_Num = (SELECT CG_Num FROM _Reglement_) 
		AND     CAST(DATEADD(DAY,A.EC_Jour-1,A.JM_Date) AS DATE) = (SELECT RG_Date FROM _Reglement_) 
	) A
)
,_PreSource_ AS (
SELECT	
