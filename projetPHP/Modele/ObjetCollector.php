...
INSERT INTO " . $this->db->baseCompta . ".dbo.[F_JMOUV]
 ...
 WHERE [CA_Num] = '$CA_Num'";
    ...
    {
        return "select D_Commentaire,D_Profession,D_RaisonSoc,D_GestionIFRS,D_Identifiant,D_Profession,D_CodePostal,D_Ville,D_Pays,D_Siret,
                        D_EmailSoc,D_Telephone from  P_DOSSIER";
    }
...
LEFT JOIN (SELECT cbMarq,DO_Piece AS DO_Piece_Dest,DL_PrixUnitaire AS DL_PrixUnitaire_Dest,AR_Ref AS AR_Ref_Dest,DL_Design AS DL_Design_Dest,DL_Qte AS DL_Qte_Dest,DL_CMUP AS DL_CMUP_Dest,
    ...
    public function getReglementByClientFacture($ct_num, $do_piece,$do_type,$do_domaine) {
        return "with cte (ligne,do_piece,ct_numpayeur,RG_No,RG_Date,DR_Date,RG_Libelle,RG_Montant,CA_No,RC_Montant,DL_MontantTTC,cumul) 
                as( 
    ...
    public function getAffaire($sommeil=-1) {
        return "SELECT '' AS CA_Num, '' AS CA_Intitule UNION SELECT CA_Num,CA_Intitule FROM ".$this->db->baseCompta.".dbo.F_COMPTEA WHERE N_Analytique=1 AND CA_Type=0 AND ($sommeil=-1 OR CA_Sommeil=$sommeil)";
    }
...
from(select  row_number() over (order by u.subject) as idcompta,u.marks
    ...
    public function getSoucheDepotGrpAffaire($prot_no,$type,$sommeil=-1){
        return "SELECT CA_Num,CA_Intitule,ISNULL(MAX(IsPrincipal),0)IsPrincipal
                    FROM(
    ...
    public function getCaisseDepot($prot_no){
        return "IF (SELECT CASE WHEN ISNULL(ProtectAdmin,0) = 0 AND (PROT_Administrator=1 OR PROT_Right=1) THEN 1 ELSE 0 END 
                    FROM F_PROTECTIONCIAL fpt
                    LEFT JOIN Z_ProtUser zdu
                        ON fpt.PROT_No = zdu.Prot_No
                    WHERE fpt.Prot_No=$prot_no) = 1 
                    SELECT	D.DE_No
    ...
    public function getSoucheAchat(){
        return "SELECT S_Intitule,(cbIndice-1) as cbIndice,JO_Num  FROM P_SOUCHEACHAT WHERE S_Intitule<>'' AND S_Valide=1 ";
    }

    public function getSoucheInterne(){
        return "SELECT S_Intitule,cbIndice-1 as cbIndice FROM P_SOUCHEINTERNE WHERE S_Intitule<>'' AND S_Valide=1 ";
    }
...
GROUP BY A.CA_No,A.CA_Intitule,B.NB
    ...
    function getComptegCount() {
        return "SELECT COUNT(*) Nb,MAX(cbModification) cbModification FROM F_COMPTEG WHERE CG_Type=0 ORDER BY cbModification DESC";
    }
    ...
    function getModeleReglement() {
        return "SELECT MR_No,MR_Intitule,cbModification
