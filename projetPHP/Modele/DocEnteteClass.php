...
,_docEntete_ AS (
	SELECT	nomFichier =''
			...
			,EC_Sens =	CASE WHEN docE.DO_Domaine = 0 AND cptT.CT_Num IS NULL THEN 
									CASE WHEN docE.DO_Provenance IN (1,2) AND @pLigneNeg = 1 THEN 0 ELSE 1 END
							 WHEN docE.DO_Domaine = 1 AND cptT.CT_Num IS NOT NULL THEN 
									CASE WHEN DO_Provenance IN (1,2) AND @pLigneNeg = 1 THEN 0 ELSE 1 END
						ELSE 
							CASE WHEN DO_Provenance IN (1,2) AND @pLigneNeg = 1 THEN 1 ELSE 0 END
						END
			,EC_StatusRegle = CASE WHEN abs(mor.montantRegle - ava.avance_regle)<5 THEN 2 ELSE 0 END
			,EC_MontantRegle = CASE WHEN ava.avance_regle<>0 THEN ava.avance_regle ELSE mor.montantRegle END
			...
			,CT_NumCont = ''
			,EC_Intitule = docE.DO_Piece +'_'+docE.DO_Ref
			,N_Reglement = 1
...
,_Ligne_ AS (
	SELECT	nomFichier,JO_Num,DO_Date,Annee_Exercice,EC_Jour,EC_RefPiece
			...
			,EC_Echeance = docR.DR_Date,TA_Code,CT_Lettrage,N_CatCompta,docL.DO_Domaine
			,docL.DO_Type,docL.DO_Piece,docL.cbDO_Piece,docL.CbMarq
			,docR.DR_Montant
			,docR.DR_MontantTotal
...
,_LigneTaxe_ AS (
		...
		SELECT nomFichier
				...
				,EC_Echeance
				,TA_Code
				,EC_MontantCredit = SUM(EC_MontantCredit)
				,EC_MontantDebit = SUM(EC_MontantDebit)
				,CbMarq
		FROM (
		SELECT nomFichier =''
				,JO_Num
...
, _LettrageLigne_ AS (
	SELECT  lettrage = ISNULL(lettrage,'A')
	FROM (
		SELECT  lettrage = CHAR(ASCII(EC_Lettrage)+1)
		FROM    F_ECRITUREC A
		WHERE   EC_Lettre=1
...
";
    ...
    {
        $query = "SELECT CASE WHEN CAST(SUM(DL_MontantHT) AS MONEY)=CAST(SUM(EA_Montant) AS MONEY) AND SUM(EA_Quantite)=SUM(DL_Qte) THEN 1 ELSE 0 END AS VALID
                FROM(
                SELECT  B.CbMarq_Ligne,N_Analytique
    ...
    public function setDO_Modif(){
        $query="SELECT CASE WHEN ABS(DATEDIFF(d,GETDATE(),DO_Date))>= (select PR_DelaiPreAlert
    ...
    public function transformBL_Dev_Facture ($conserv,$canTransform,$type_trans,$ref,$type){
        ...
        if(Sizeof($this->getStatutVente($type_res))>0) {
            ...
            foreach ($rows as $elt) {
                ...
                $arRef = new ArticleClass($docligne->AR_Ref);
    ...
    public function redirectToNouveau($type){
        $element = array();
        if($type=="Vente" || $type=="VenteC" || $type=="VenteT")
            $element = array('action' => 3,'module' => 2);
        if($type=="Achat" || $type=="AchatC" || $type=="AchatT")
            $element = array('action' => 2,'module' => 7);
        if($type=="AchatRetour" || $type=="AchatRetourC" || $type=="AchatRetourT")
            $element = array('action' => 2,'module' => 7);
        if($type=="BonLivraison")
            $element = array('action' => 6,'module' => 2);
        if($type=="Retour")
            $element = array('action' => 10,'module' => 2);
        if($type=="Avoir")
            $element = array('action' => 8,'module' => 2);
        if($type=="AchatPreparationCommande")
            $element = array('action' => 6,'module' => 7);
        if($type=="PreparationCommande")
            $element = array('action' => 4,'module' => 7);
        if($type=="Devis")
            $element = array('action' => 4,'module' => 2);
        if($type=="Entree")
            $element = array('action' => 7,'module' => 4);
        if($type=="Sortie")
            $element = array('action' => 8,'module' => 4);
        if($type=="Transfert")
            $element = array('action' => 5,'module' => 4);
        if($type=="Transfert_detail")
            $element = array('action' => 10,'module' => 4);
        if($type=="Transfert_confirmation")
            $element = array('action' => 12,'module' => 4);
        if($type=="Ticket")
            $element = array('action' => 14,'module' => 2);
        return "indexMVC.php?module=".$element["module"]."&action=".$element["action"]."&type=$type";
    }

    public function getListeFacture($de_no,$do_provenance, $datedeb, $datefin,$client,$do_domaine,$do_type,$protNo){
        $query = "  WITH _Depot_ AS (
                    ...
                    AND ((0=$de_no AND E.DE_No IN (SELECT DE_No FROM  _Depot_)) OR E.DE_No =$de_no)  
...
public function setValueMvtEntree (){
    ...
    public function listeTransfert($do_tiers, $datedeb, $datefin){
        $query = "SELECT    PROT_User,'' DL_PieceBL,DO_Imprim
                            ,CASE WHEN ABS(DATEDIFF(d,GETDATE(),E.DO_Date))>= (select PR_DelaiPreAlert from P_PREFERENCES) THEN 1 ELSE 0 END DO_Modif
    ...
    public function listeTransfertConfirmation($do_tiers, $datedeb, $datefin){
        $query = "SELECT PROT_User,'' DL_PieceBL,DO_Imprim,CASE WHEN ABS(DATEDIFF(d,GETDATE(),E.DO_Date))>= (select PR_DelaiPreAlert
from P_PREFERENCES) THEN 1 ELSE 0 END DO_Modif,E.cbModification,E.cbMarq,E.DO_Type,E.DO_Domaine,E.DO_Piece,E.DO_Ref,ISNULL(MAX(E.N_CatCompta),1) N_CatCompta,CAST(CAST(E.DO_Date AS DATE) AS VARCHAR(10)) AS DO_Date,E.DO_Tiers as CT_Num, ISNULL(SUM(L.DL_MontantTTC),0) AS ttc,
    ...
    public function listeEntree($do_tiers, $datedeb, $datefin){
        $query = "SELECT  PROT_User,DO_Imprim,DL_PieceBL
                          ,CASE WHEN ABS(DATEDIFF(d,GETDATE(),E.DO_Date))>= (select PR_DelaiPreAlert from P_PREFERENCES) THEN 1 ELSE 0 END DO_Modif
                          ,E.cbModification,E.cbMarq,E.DO_Type,ISNULL(MAX(E.N_CatCompta),1) N_CatCompta,E.DO_Domaine,E.DO_Piece
                          ,E.DO_Ref,CAST(CAST(E.DO_Date AS DATE) AS VARCHAR(10)) AS DO_Date,E.DO_Tiers AS CT_Num
                          ,ISNULL(SUM(L.DL_MontantTTC),0) AS ttc
                          ,MAX(DE_Intitule) DE_Intitule,'' CT_Intitule,ISNULL(MAX(latitude),0) AS latitude
                          ,ISNULL(MAX(longitude),0) AS longitude,0 AS avance
                          ,'' AS statut
                 FROM F_DOCENTETE E
                 INNER JOIN F_DEPOT C 
                    ON C.DE_No=E.DO_Tiers
                 LEFT JOIN F_DOCLIGNE L 
                    ON  E.cbDO_Piece=L.cbDO_Piece 
                    AND E.DO_Domaine= L.DO_Domaine 
                    AND E.DO_Type=L.DO_Type
                 LEFT JOIN F_PROTECTIONCIAL P 
                     ON E.cbCreateur = CAST(P.PROT_No AS VARCHAR(5))
                WHERE   E.DO_Domaine=2 
                AND     E.DO_Type=20 
                AND     ('0'='$do_tiers' OR E.DO_Tiers ='$do_tiers')
                AND     CAST(E.DO_Date as DATE) BETWEEN '$datedeb' AND '$datefin'
                GROUP BY PROT_User,DL_PieceBL,DO_Imprim,E.cbModification,E.cbMarq,E.DO_Type,E.DO_Domaine,E.DO_Piece,E.DO_Ref,E.DO_Date,E.DO_Tiers
                ORDER BY E.cbMarq ";
        $result= $this->db->query($query);
        return $result->fetchAll(PDO::FETCH_OBJ);
    }
                                    ...
                                    CASE WHEN cbIndice=3 THEN 'PreparationLivraison' ELSE
                                            CASE WHEN cbIndice=4 THEN 'BonLivraison' ELSE 
                                                    CASE WHEN cbIndice=5 THEN 'BonRetour' ELSE
                                                            ...
                                                            CASE WHEN cbIndice=8 THEN 'Retour' ELSE 
    ...
    public function getLigneTransfert(){
        $query= "SELECT ISNULL(idSec,0)idSec,A.*
                ...
                LEFT JOIN (SELECT	DL_Ligne Ligne,idSec = cbMarq,AR_Ref
    ...
    public function addDocenteteFactureProcess($CT_Num, $CO_No, $ref, $date, $N_Reglement,$de_no,$cat_tarif,$cat_compta,$souche,$ca_no,$affaire,$type_fac,$do_statut,$latitude,$longitude,$date_ech,$do_piece,$DO_Cood2,$DO_Coord03,$DO_Coord04,$mobile,$user,$transform=0) {
        ...
        if($html !="" && $this->db->user!="sa") {
            $mail = new Mail();
            $mail->sendMail($html."<br/><br/><br/> {$this->db->db}","info@it-solution-sarl.com","Liste des transferts à une ligne");
        }
...
";
    ...
    public function montantRegle() {
        $query = "
                    SELECT  ROUND(ISNULL(SUM(DL_MontantTTC),0),2) montantRegle
                    FROM    F_DOCENTETE E                    
                    LEFT JOIN F_DOCLIGNE D 
                        ON  E.cbDO_Piece = D.cbDO_Piece 
                        AND E.DO_Domaine = D.DO_Domaine 
        ...
        $rowsTour = $result->fetchAll(PDO::FETCH_OBJ);
        if($rowsTour!=null)
            return $rowsTour[0]->montantRegle;
        return 0;
    }

    public function AvanceDoPiece() {
        $query = "SELECT    ROUND(ISNULL(SUM(RC_Montant),0),2) avance_regle
                    FROM    F_DOCENTETE E                    
                    LEFT JOIN F_REGLECH D 
                        ON  E.cbDO_Piece = D.cbDO_Piece 
                        AND E.DO_Domaine = D.DO_Domaine 
        ...
        $rowsTour = $result->fetchAll(PDO::FETCH_OBJ);
        if($rowsTour!=null)
            return $rowsTour[0]->avance_regle;
        return 0;
    }

    public function isVisu($PROT_Administrator,$protectedDocP,$flagProtApresImpressionP,$isSecurite)
    {
        if($isSecurite==0)
            return 1;
        else
    ...
    public function isModif($PROT_Administrator,$PROT_Right,$protectedDocP,$flagProtApresImpressionP,$isSecurite){
        if($isSecurite==0)
            return 0;
        else
    ...
    public function getEnteteByDOPiece($dopiece) {
        $result = $this->db->requete("SELECT *,CONVERT(char(10), CAST(DO_Date AS DATE),126) AS DO_DateC 
                ...
                AND     DO_Domaine='{$this->DO_Domaine}' 
                AND     (({$this->DO_Type}=6 AND (DO_Type IN(6,7))) 
                            OR ({$this->DO_Type}=16 AND (DO_Type IN(16,17))) 
                                OR (DO_Type NOT IN (16,6) AND {$this->DO_Type}=DO_Type ))");
        $rowsTour = $result->fetchAll(PDO::FETCH_OBJ);
        if($rowsTour!=null)
...
from P_PREFERENCES) THEN 1 ELSE 0 END DO_Modif,E.cbMarq,E.DO_Type,E.DO_Domaine,E.DO_Piece,E.cbDO_Piece,E.DO_Ref,CAST(CAST(E.DO_Date AS DATE) AS VARCHAR(10)) AS DO_Date,E.DO_Tiers as CT_Num,E.DE_No,DE_Intitule
                    ...
                    AND     E.DO_Type=41 
                    AND     ('0'='$do_tiers' OR E.DE_No ='$do_tiers') 
                    AND     CAST(E.DO_Date as DATE) BETWEEN '$datedeb' AND '$datefin'
