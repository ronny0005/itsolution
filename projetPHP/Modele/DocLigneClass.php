...
class DocLigneClass Extends Objet
{
    ...
    function __construct($id, $db = null)
    {
        ...
        if (sizeof($this->data) > 0) {
            ...
            $this->cbDT_No = $this->data[0]->cbDT_No;
    ...
    {
        $query = "SELECT [DL_Remise] = CASE WHEN DL_Remise01REM_Type=0 THEN ''  
...
//        $article = new ArticleClass($this->AR_Ref);
        ...
        $requete = "
            ...
            VALUES
                ...
                ,/*DL_DateBL*/'{$this->DL_DateBL}',/*DL_Ligne*/ (SELECT (1+COUNT(*))*10000 FROM F_DOCLIGNE WHERE cbDO_PIECE='{$this->DO_Piece}' AND DO_Domaine={$this->DO_Domaine} AND DO_Type={$this->DO_Type} ),/*DO_Ref*/'{$this->DO_Ref}',/*DL_TNomencl*/{$this->DL_TNomencl}
               ...
               ,/*DL_Remise03REM_Type*/{$this->DL_Remise03REM_Type},/*DL_PrixUnitaire*/{$this->DL_PrixUnitaire}
                ...
                ,/*DL_TypeTaxe2*/{$this->DL_TypeTaxe2},/*CO_No*/{$this->CO_No},/*AG_No1*/{$this->AG_No1}
               ,/*AG_No2*/{$this->AG_No2},/*DL_PrixRU*/{$this->DL_PrixRU},/*DL_CMUP*/{$this->DL_CMUP},/*DL_MvtStock*/{$this->DL_MvtStock}
                ,/*DT_No*/{$this->DT_No},/*AF_RefFourniss*/'{$this->AF_RefFourniss}'
                ,/*EU_Enumere*/'{$this->EU_Enumere}',/*EU_Qte*/{$this->EU_Qte},/*DL_TTC*/{$this->DL_TTC},/*DE_No*/{$this->DE_No},/*DL_NoRef*/{$this->DL_NoRef}
    ...
    public function verifbornePrix($rows, $type,$protNo,$catTarif,$arRef)
    {
        if (isset($_SESSION)) {
            ...
            if ((($type == "Vente" || $type == "Devis" || $type == "BonLivraison") && $protection->PROT_Right != 1 && $flag_minMax == 1)
                || ($rows->Prix_Min == 0 && $rows->Prix_Max == 0))
            {
                ...
                if($pCommunication->N_CatTarif ==3){
                    if($catTarif>=2){
                        ...
                        $min = $artClient->AC_Coef;
                        $max = $artClient->AC_PrixVen;
                    }
                }
                if (($min != 0 && $max != 0 ) &&
    ...
    public function addDocligneFactureProcess($dl_mvtStock, $AR_Ref, $DL_Qte, $remise, $type_remise, $cat_tarif, $prix, $login, $type_fac, $machine, $cbMarqEntete, $protNo, $entete_prev = "")
    {
        ...
        if ($rows != null) {
            ...
            if (strcmp($type_fac, "PreparationCommande") != 0)
                if ($AS_QteSto > 0){
                    ...
                    if($type_fac!="Retour" && $type_fac!="Entree")
                        $AR_PrixAch = 0;
                }
            $AS_MontSto = 0;
        }
        ...
        if (strcmp($type_fac, "PreparationCommande") != 0 && strcmp($type_fac, "Devis") != 0 && strcmp($type_fac, "Avoir") != 0) {
...
//        $this->modifiePrix($AR_Ref, $AR_Design, $AR_PrixAch, $AR_PrixVen, $prix, $pxMin, $docEntete->DO_Piece, $login);
    ...
    public function ajout_ligneFacturation($qteG, $ARRefG, $cbMarqEntete, $typeFacG, $cattarifG, $prixG, $remiseG, $machinepcG, $acte,$protNo)
    {
        $login = "";
        $VteNegatif = 0;
        $this->DL_Qte = ($this->DL_Qte == "") ? 0 : $this->DL_Qte;
        if ($acte != "ajout_ligne") {
            $ARRefG = $this->AR_Ref;
        }
        $docEntete = new DocEnteteClass($cbMarqEntete, $this->db);
        $article = new ArticleClass($ARRefG, $this->db);
        ...
        if ($isStock == null)
            ...
            $cbMarqIsStock = $isStock[0]->cbMarq;
        }
        $result = $this->db->requete($this->objetCollection->getPPreference());
        $rows = $result->fetchAll(PDO::FETCH_OBJ);
        foreach ($rows as $row) {
            $VteNegatif = $row->PR_StockNeg;
        }
        if ($typeFacG != "Vente" || ($article->Qte_Gros != 0 && (($article->Qte_Gros <= $qteG) || ($article->Qte_Gros > $qteG && $article->Prix_Min < $prixG))) || $article->Qte_Gros == 0)
        {
            if ($typeFacG == "Achat" || $typeFacG == "AchatPreparationCommande"
                || ($typeFacG != "Achat"
                    && (($VteNegatif == 1 && ($typeFacG == "BonCommande" || $typeFacG == "PreparationLivraison" || $typeFacG == "BonLivraison"))
                        || (/*$VteNegatif==0 &&*/ ($AS_QteSto + $this->DL_Qte) >= ($qteG)))) || $typeFacG == "Devis" || $typeFacG == "PreparationCommande" || $typeFacG == "Avoir" || $typeFacG == "Retour") {
                $qte = str_replace(",", ".", $qteG);

                $ref_article = $ARRefG;
                $cat_tarif = $cattarifG;
                $prix = str_replace(",", ".", $prixG);
                $aprix = str_replace(",", ".", $prixG);

                $remise = 0;
                if ($remiseG != "undefined" && $remiseG != "")
                    $remise = $remiseG;
                $ADL_Qte = $this->DL_Qte;
                $type_fac = $typeFacG;
                $mvtStock = 1;
                if ($qte >= 0 && $type_fac != "Retour") {
                    $mvtStock = 3;
                }
                $type_remise = 0;
                ...
                if (isset($machinepcG))
                    $machine = $machinepcG;
                $resultData = "";
                if ($type_fac != "Achat" && $type_fac != "AchatC" && $type_fac != "AchatRetour" && $type_fac != "PreparationCommande" && $type_fac != "AchatPreparationCommande") {
                    if ($acte == "ajout_ligne") {
                        $resultData = $this->addDocligneFactureProcess($mvtStock, $ref_article, $qte, $remise, $type_remise, $cat_tarif, $prix, $login, $type_fac, $machine, $cbMarqEntete, $protNo);
                    } else {
                        $resultData = $this->modifDocligneFacture($ref_article, $qte, $remise, $type_remise, $this->cbMarq, $cbMarqEntete, $cat_tarif, $prix, $login, $type_fac, $machine,$protNo);
                    }
                } else {
                    $docEntete = new DocEnteteClass($cbMarqEntete, $this->db);
                    if ($acte == "ajout_ligne") {
                        $resultData = $this->addDocligneAchatProcess(1, $cbMarqEntete, $ref_article, $qte, $remise, $type_remise, $cat_tarif, $prix, $type_fac, $machine, $protNo);
                    } else {
                        $resultData = $this->modifDocligneAchat($cbMarqEntete, $ref_article, $qte, $remise, $type_remise, $cat_tarif, $prix, $type_fac, $machine, $protNo);
                    }
                }
                return $resultData;
            }
            else {
                $data = array('message' => "La quantité de " . $ARRefG . " ne doit pas dépasser " . ROUND($AS_QteSto + $this->DL_Qte, 2) . " !");
                return json_encode($data);
            }
        }
        else{
            $data = array('message' => "La quantité saisie de {$ARRefG} ({$this->objetCollection->formatChiffre($qteG)}) < quantité gros ({$this->objetCollection->formatChiffre($article->Qte_Gros)})");
            return json_encode($data);
        }
    }
    ...
    {
        $log = new LogFile();
        $log->writeFacture($action, $this->DO_Type, $this->DO_Piece, $DE_No, $this->DO_Domaine, $AR_Ref, $Qte, $Prix, $Remise, $Montant,$cbMarq,"F_DOCLIGNE",$_SESSION["id"],$cbCreateur,$doDate);
    }
...
//        $this->logMvt("ajout_ligne", $cbMarqEntete, 0, $AR_Ref, $DL_Qte, $this->DL_PrixUnitaire, 0, $this->DL_MontantTTC,$protNo,$cbmarqligne,'F_DOCLIGNE',$protNo);
    ...
    public function modifDocligneFacture($AR_Ref, $DL_Qte, $remise, $type_remise, $cbMarq, $cbMarqEntete, $cat_tarif, $prix, $login, $type_fac, $machine, $protNo, $entete_prev = "")
    {
        ...
        if ($rows != null) {
            ...
            if ($type_fac == "Retour" || $type_fac == "Achat" || $type_fac == "AchatPreparationCommande" || $type_fac == "PreparationCommande") {
            ...
            if (strcmp($type_fac, "Devis") != 0 && strcmp($type_fac, "Avoir") != 0) {
