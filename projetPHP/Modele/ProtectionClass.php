...
class ProtectionClass {
    ...
    function __construct($nom,$mdp,$db=null)
    {
        if($db==null)
            $this->db = new DB();
        else
            $this->db = $db;
        $rows = $this->connexionProctection($nom, $mdp);
        $this->initParam($rows);
    }
    ...
    public function initParam($rows)
    {
        ...
        if (sizeof($rows) > 0) {
            ...
            $this->PROT_User = $rows [0]->PROT_User;
            $this->PROT_Pwd = $rows [0]->PROT_Pwd;
            $this->ProfilName = $rows [0]->ProfilName;
            $this->PROT_PwdStatus = $rows [0]->PROT_PwdStatus;
            $this->PROT_Administrator = $rows [0]->PROT_Administrator;
            $this->PROT_Description = $rows [0]->PROT_Description;
            $this->PROT_Right = $rows [0]->PROT_Right;
            $this->PROT_Email = $rows [0]->PROT_Email;
            $this->ProtectAdmin = $rows [0]->ProtectAdmin;
            $this->Prot_No = $rows [0]->Prot_No;
			$this->PROT_UserProfil = $rows [0]->PROT_UserProfil;
            $this->PROT_CLIENT = $rows [0]->PROT_CLIENT;
            $this->PROT_FOURNISSEUR = $rows [0]->PROT_FOURNISSEUR;
            $this->PROT_COLLABORATEUR = $rows [0]->PROT_COLLABORATEUR;
            $this->PROT_FAMILLE = $rows [0]->PROT_FAMILLE;
            $this->PROT_ARTICLE = $rows [0]->PROT_ARTICLE;
            $this->PROT_CATCOMPTA = $rows [0]->PROT_CATCOMPTA;
            $this->PROT_MODIFICATION_CLIENT = $rows[0]->PROT_MODIFICATION_CLIENT;
            $this->PROT_APRES_IMPRESSION = $rows [0]->PROT_APRES_IMPRESSION;
            $this->PROT_AVANT_IMPRESSION = $rows [0]->PROT_AVANT_IMPRESSION;
            $this->PROT_TICKET_APRES_IMPRESSION = $rows [0]->PROT_TICKET_APRES_IMPRESSION;
            $this->PROT_GENERATION_RGLT_CLIENT = $rows [0]->PROT_GENERATION_RGLT_CLIENT;
            $this->PROT_DOCUMENT_STOCK = $rows [0]->PROT_DOCUMENT_STOCK;
            $this->PROT_DOCUMENT_ACHAT = $rows [0]->PROT_DOCUMENT_ACHAT;
            $this->PROT_TARIFICATION_CLIENT = $rows [0]->PROT_TARIFICATION_CLIENT;
            $this->PROT_MODIF_SUPPR_COMPTOIR = $rows [0]->PROT_MODIF_SUPPR_COMPTOIR;
            $this->PROT_DOCUMENT_ACHAT_FACTURE = $rows [0]->PROT_DOCUMENT_ACHAT_FACTURE;
            $this->PROT_DOCUMENT_ACHAT_PREPARATION_COMMANDE = $rows [0]->PROT_DOCUMENT_ACHAT_PREPARATION_COMMANDE;
            $this->PROT_DOCUMENT_INTERNE_2 = $rows [0]->PROT_DOCUMENT_INTERNE_2;
            $this->PROT_DOCUMENT_VENTE = $rows [0]->PROT_DOCUMENT_VENTE;
            $this->PROT_PX_ACHAT = $rows [0]->PROT_PX_ACHAT;
            $this->PROT_DEPRECIATION_STOCK = $rows [0]->PROT_DEPRECIATION_STOCK;
            $this->PROT_PX_REVIENT = $rows [0]->PROT_PX_REVIENT;
            $this->PROT_DATE_STOCK = $rows [0]->PROT_DATE_STOCK;
            $this->PROT_DATE_VENTE = $rows [0]->PROT_DATE_VENTE;
            $this->PROT_DATE_ACHAT = $rows [0]->PROT_DATE_ACHAT;
            $this->PROT_DATE_COMPTOIR = $rows [0]->PROT_DATE_COMPTOIR;
            $this->PROT_VENTE_COMPTOIR= $rows [0]->PROT_VENTE_COMPTOIR;
            $this->PROT_DATE_RGLT = $rows [0]->PROT_DATE_RGLT;
            $this->PROT_GEN_ECART_REGLEMENT = $rows [0]->PROT_GEN_ECART_REGLEMENT;
            $this->PROT_ETAT_STAT_CAISSE_ARTICLE = $rows [0]->PROT_ETAT_STAT_CAISSE_ARTICLE;
            $this->PROT_ETAT_STAT_CAISSE_FAM_ARTICLE = $rows [0]->PROT_ETAT_STAT_CAISSE_FAM_ARTICLE;
            $this->PROT_ETAT_CAISSE_MODE_RGLT = $rows [0]->PROT_ETAT_CAISSE_MODE_RGLT;
            $this->PROT_DOCUMENT_VENTE_DEVIS = $rows [0]->PROT_DOCUMENT_VENTE_DEVIS;
            $this->PROT_DOCUMENT_VENTE_FACTURE = $rows [0]->PROT_DOCUMENT_VENTE_FACTURE;
            $this->PROT_DOCUMENT_VENTE_BLIVRAISON = $rows [0]->PROT_DOCUMENT_VENTE_BLIVRAISON;
            $this->PROT_DOCUMENT_VENTE_RETOUR = $rows [0]->PROT_DOCUMENT_VENTE_RETOUR;
            $this->PROT_DOCUMENT_VENTE_AVOIR = $rows [0]->PROT_DOCUMENT_VENTE_AVOIR;
            $this->PROT_DOCUMENT_ACHAT_RETOUR = $rows [0]->PROT_DOCUMENT_ACHAT_RETOUR;
            $this->PROT_DOCUMENT_ENTREE = $rows [0]->PROT_DOCUMENT_ENTREE;
            $this->PROT_DOCUMENT_SORTIE = $rows [0]->PROT_DOCUMENT_SORTIE;
            $this->PROT_DOCUMENT_REGLEMENT = $rows [0]->PROT_DOCUMENT_REGLEMENT;
            $this->PROT_MVT_CAISSE = $rows [0]->PROT_MVT_CAISSE;
            $this->PROT_QTE_NEGATIVE = $rows [0]->PROT_QTE_NEGATIVE;
            $this->PROT_SAISIE_INVENTAIRE = $rows[0]->PROT_SAISIE_INVENTAIRE;
            $this->PROT_SAISIE_REGLEMENT = $rows[0]->PROT_SAISIE_REGLEMENT;
            $this->PROT_SAISIE_REGLEMENT_FOURNISSEUR = $rows [0]->PROT_SAISIE_REGLEMENT_FOURNISSEUR;
            $this->PROT_DEPOT = $rows [0]->PROT_DEPOT;
            $this->PROT_RISQUE_CLIENT = $rows [0]->PROT_RISQUE_CLIENT;
            $this->PROT_AFFICHAGE_VAL_CAISSE = $rows [0]->PROT_AFFICHAGE_VAL_CAISSE;
            $this->PROT_CTRL_TT_CAISSE = $rows [0]->PROT_CTRL_TT_CAISSE;
            $this->PROT_INFOLIBRE_ARTICLE = $rows [0]->PROT_INFOLIBRE_ARTICLE;
            $this->PROT_DATE_MVT_CAISSE = $rows [0]->PROT_DATE_MVT_CAISSE;
            $this->PROT_ETAT_INVENTAIRE_PREP = $rows [0]->PROT_ETAT_INVENTAIRE_PREP;
            $this->PROT_ETAT_LIVRE_INV = $rows [0]->PROT_ETAT_LIVRE_INV;
            $this->PROT_ETAT_STAT_ARTICLE_PAR_ART = $rows [0]->PROT_ETAT_STAT_ARTICLE_PAR_ART;
            $this->PROT_ETAT_STAT_ARTICLE_PAR_FAM = $rows [0]->PROT_ETAT_STAT_ARTICLE_PAR_FAM;
            $this->PROT_ETAT_STAT_ARTICLE_PALMARES = $rows [0]->PROT_ETAT_STAT_ARTICLE_PALMARES;
            $this->PROT_ETAT_MVT_STOCK = $rows [0]->PROT_ETAT_MVT_STOCK;
            $this->PROT_ETAT_CLT_PAR_FAM_ART = $rows [0]->PROT_ETAT_CLT_PAR_FAM_ART;
            $this->PROT_ETAT_CLT_PAR_ARTICLE = $rows [0]->PROT_ETAT_CLT_PAR_ARTICLE;
            $this->PROT_ETAT_PALMARES_CLT = $rows [0]->PROT_ETAT_PALMARES_CLT;
            $this->PROT_ETAT_STAT_FRS_FAM_ART = $rows [0]->PROT_ETAT_STAT_FRS_FAM_ART;
            $this->PROT_ETAT_STAT_FRS = $rows [0]->PROT_ETAT_STAT_FRS;
            $this->PROT_ETAT_RELEVE_CPTE_CLIENT = $rows [0]->PROT_ETAT_RELEVE_CPTE_CLIENT;
            $this->PROT_ETAT_STAT_COLLAB_PAR_TIERS = $rows [0]->PROT_ETAT_STAT_COLLAB_PAR_TIERS;
            $this->PROT_ETAT_STAT_COLLAB_PAR_ARTICLE = $rows [0]->PROT_ETAT_STAT_COLLAB_PAR_ARTICLE;

            $this->PROT_ETAT_STAT_COLLAB_PAR_FAMILLE = $rows [0]->PROT_ETAT_STAT_COLLAB_PAR_FAMILLE;
            $this->PROT_ETAT_RELEVE_ECH_CLIENT = $rows [0]->PROT_ETAT_RELEVE_ECH_CLIENT;
            $this->PROT_ETAT_RELEVE_ECH_FRS = $rows [0]->PROT_ETAT_RELEVE_ECH_FRS;
            $this->PROT_SAISIE_PX_VENTE_REMISE = $rows [0]->PROT_SAISIE_PX_VENTE_REMISE;
            $this->PROT_CBCREATEUR = $rows [0]->PROT_CBCREATEUR;
            $this->PROT_CLOTURE_CAISSE = $rows [0]->PROT_CLOTURE_CAISSE;
            $this->PROT_OUVERTURE_TOUTE_LES_CAISSES = $rows [0]->PROT_OUVERTURE_TOUTE_LES_CAISSES;
        }
    }
    ...
    public function connexionProctectionByProtNo($prot_no){
        ...
        $rows = $result->fetchAll(PDO::FETCH_OBJ);
        if(sizeof($rows)>0) {
            $user = $rows[0]->PROT_User;
            $mdp = $rows[0]->PROT_Pwd;
            $protect = $this->connexionProctection($user, $mdp);
            $this->initParam($protect);
        }
    }
    ...
    public function IssecuriteAdmin($deNo){
        ...
        if($this->ProtectAdmin==1) {
            $depotUser = new DepotUserClass(0);
            $rows = $depotUser->getDepotPrincipal($_SESSION["id"]);
            foreach ($rows as $row) {
                if ($row->DE_No == $deNo)
                    $isSecurite = 1;
            }
    ...
    public function getDelai(){
        $query="SELECT PR_DelaiPreAlert
                FROM P_PREFERENCES";
        $result= $this->db->query($query);
        $rows = $result->fetchAll(PDO::FETCH_OBJ);
        if($this->PROT_Right==1)
            return -1;
        else
            return $rows[0]->PR_DelaiPreAlert;
    }
...
public function getUtilisateurAdminMainConnexion(){
    ...
    public function canConnect($PROT_No,$jour,$heure){
        $query = "SELECT CASE WHEN CAST('$heure' AS TIME) BETWEEN HeureDebut AND HeureFin THEN 1 ELSE 0 END as canConnect
                    FROM (SELECT PROT_No 
								,Id_JourDebut	
								,ID_JourFin
                    ...
                    WHERE PROT_No = $PROT_No AND Id_JourDebut=$jour";
        echo $query;
        $result= $this->db->query($query);
        $row = $result->fetchAll(PDO::FETCH_OBJ);
        if($row!=null) {
            return $row[0]->canConnect;
        }
        return 0;
    }
...
FROM(
        ...
        WHERE (PROT_Right<>0 OR (PROT_Right=0 AND b.EPROT_Cmd IN (	'33541','33537','33538','34051','34049'
                                ...
                                ,'12133','36736','36737','36738','36357','36358','5122'
        ...
        FOR EPROT_Cmd IN (	[33541],[33537],[33538],[34051],[34049],[6150],[6145],[34050],[8193],[8194]
                            ...
                            ,[36356],[36688],[36689],[36690],[34306],[12130],[33068],[5122]
