...
class ComptetClass Extends Objet{
    ...
    {
        parent::__construct($this->table, $id, 'cbCT_Num',$db);
        if($id==NULL) {
            $this->CT_Num = "";
            $this->CT_Intitule = "";
            $this->CT_Type = "";
...
//        parent::maj("CA_NumIFRS" , $this->CA_NumIFRS);
    ...
    public function controleEncours($typeFacG,$prixG,$acte){
        if($typeFacG=="Vente" || $typeFacG=="BonLivraison" || $typeFacG=="Retour"){
...
//        if($select==-1)
            $info = " UNION SELECT *
                            ...
                            WHERE CONCAT(CONCAT(CT_Num,' - '),CT_Intitule) LIKE '%{$value}%'";
        $query="SELECT TOP 10 CT_Num
                            ,CT_Intitule
                            ,CO_No
                            ,N_CatCompta
                            ,N_CatTarif
                FROM F_COMPTET
                WHERE CT_Type = $type AND CONCAT(CONCAT(CT_Num,' - '),CT_Intitule) LIKE '%{$value}%'
                AND ($CT_Sommeil =-1 OR CT_Sommeil = $CT_Sommeil) 
                $info ";
        $result= $this->db->query($query);
        $rows = array();
        $value = $result->fetchAll(PDO::FETCH_OBJ);
        foreach ($value as $val){
            $rows[] = array("label" => $val->CT_Intitule,"CT_Intitule" => $val->CT_Intitule,
            "value" => $val->CT_Num,"CT_Num" => $val->CT_Num,"CO_No" => $val->CO_No,"N_CatCompta" => $val->N_CatCompta,'N_CatTarif' => $val->N_CatTarif);
        }
        return $rows;
    }
