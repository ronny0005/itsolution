...
class CollaborateurClass Extends Objet{
    ...
    public function allCaissier(){
        ...
        {
            array_push($this->list,new CollaborateurClass($resultat->CO_No));
        }
        return $this->list;
    }
    ...
    public function allAcheteur($type){
        $query = "  SELECT CO_No, CO_Nom 
                    FROM F_COLLABORATEUR   
                    WHERE CO_ACHETEUR=1/* AND CO_Vendeur=1 */
                    ORDER BY CO_Nom";
        $result= $this->db->query($query);
        $this->list = array();
        foreach ($result->fetchAll(PDO::FETCH_OBJ) as $resultat)
    ...
    public function allVendeur(){
        $query = "  SELECT CO_No, CO_Nom 
                    FROM ".$this->db->baseCompta.$this->table."     
                    WHERE CO_Vendeur=1 
                    ORDER BY CO_Nom";
        $result= $this->db->query($query);
        $this->list = array();
