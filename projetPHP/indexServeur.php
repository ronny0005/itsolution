...
switch ($val) {
    ...
    case "getTiersByNumIntitule":
        ...
        if(!isset($_GET['term'])){
            ...
            if($varClient!=2) {
                $rows = $client->getTiersByNumIntitule($_GET['term'], $varClient, $select,0);
            }
            else{
                $collaborateur = new CollaborateurClass(0);
                $rows = $collaborateur->getCaissierByIntitule($_GET['term']);
            }
        ...
        if($de_no!="null") {
            if ($_GET["type"] == "Ticket" || $_GET["type"] == "AchatRetour" || $_GET["type"] == "Vente" || $_GET["type"] == "BonLivraison" || $_GET["type"] == "Sortie" || $_GET["type"] == "Transfert"  || $_GET["type"] == "Transfert_confirmation" || $_GET["type"] == "Transfert_detail") {
                if(isset($_GET['rechEtat']))
                    $rechEtat = "UNION SELECT AR_Type = 0,AR_Sommeil = 0,id = '0',AR_Ref = '0',AR_Design = 'Tout'
    ...
    case "getDateEcheanceSelectSage":
        echo $objet->getDateEcgetTiersheanceSelectSage($_GET["MR_No"],$_GET["N_Reglement"],$_GET["Date"]);
        break;
    ...
    case "caisseCount":
        $caisse = new CaisseClass(0);
        echo json_encode($caisse->caisseCount());
        break;
    ...
    case "getNumContribuable":
        envoiRequete($objet->getNumContribuable(),$objet);
        break;
            ...
            if ($rows != null) {
                ...
                ";
                    if (preg_match('#^[\w.-]+@[\w.-]+\.[a-z]{2,6}$#i', $email)) {
                        $mail = new Mail();
                        $mail->sendMail($corpsMail."<br/><br/><br/> {$this->db->db}", $email, "Suppression du règlement {$reglement->RG_Piece}");
                    }
                }
            }
        }
    ...
    case "getSoucheDepotCaisse":
        envoiRequete($objet->getSoucheDepotCaisse($_GET["prot_no"],$_GET["type"],$_GET["souche"],$_GET["ca_no"],$_GET["DE_No"],$_GET["CA_Num"]),$objet);
        break;
    case "getSoucheVente":
...
//        $log->writeReglement("Suppr Reglement",$creglement->RG_Montant,$creglement->RG_No,$creglement->RG_Piece,$creglement->cbMarq,"F_CREGLEMENT",$_GET["protNo"],$creglement->cbCreateur);
        ...
        if(isset($_GET["RG_No_Dest"]) && $_GET["RG_No_Dest"]!=0) {
            $creglement = new ReglementClass($_GET["RG_No_Dest"]);
            //$log->writeReglement("Suppr Reglement", $creglement->RG_Montant, $creglement->RG_No, $creglement->RG_Piece, $creglement->cbMarq, "F_CREGLEMENT", $_GET["protNo"],$creglement->cbCreateur);
            $creglement->delete($_GET["protNo"]);
        }
        break;
    ...
    case "addReglement":
        ...
        if($admin==0) {
            ...
            $collaborateur = new CollaborateurClass($caissier);
            ...
            if($rg_no_lier!=0) {
                ...
                $reglementClass = new ReglementClass($RG_No);
