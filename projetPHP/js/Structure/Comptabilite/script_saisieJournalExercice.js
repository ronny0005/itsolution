    ...
    $("#CT_Num").select2({
        ...
        $.ajax({
            ...
            success: function (data) {
                $("#CG_Num").val(data.CG_NumPrinc).trigger("change");
            }
        });
    });
...
function inputMask() {
    ...
    function calculEnteteJournal(){
        ...
        getTotalJournal(2)
        getJournalLastDate()
        var debit = $("#totalJournalDebit").val().replace(/ /g, "");
        var credit = $("#totalJournalCredit").val().replace(/ /g, "");
    ...
    function ajoutLigne(){
        var val=true;
        if($("#CG_Num").val()=="" || $("#CT_Num").val()=="" || ($("#EC_MontantDebit").val()=="" && $("#EC_MontantCredit").val()=="") )
            val=false;
        if(val){
            sendAll();
    ...
    function clear(){
        ...
        $("#CT_Num").val(null).trigger("change");
        $("#CG_Num").val(null).trigger("change");
        $("#EC_Piece").val("");
    ...
    function getTotalJournal(ecSens){
        $.ajax({
            url: 'indexServeur.php?page=getTotalJournal',
            method: 'GET',
            dataType: 'json',
            data : 'JO_Num='+$("#Journal_JO_Num").val()+'&Mois='+$("#Journal_Mois").val()+'&Annee='+$("#Journal_Annee").val()+'&EC_Sens='+ecSens,
            async : false,
            success: function(data) {
                if(ecSens==1)
                    $("#totalJournalDebit").val(data.EC_Montant)
                if(ecSens==0)
                    $("#totalJournalCredit").val(data.EC_Montant)
                if(ecSens==2){
                    if(data.EC_Montant>0)
                        $("#EC_MontantDebit").val(data.EC_Montant)
                    else
                        $("#EC_MontantCredit").val(Math.abs(data.EC_Montant))
