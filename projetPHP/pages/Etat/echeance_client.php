...
$depotClass = new DepotClass(0);
    ...
    foreach($rows as $row){
        ...
        $result=$objet->db->requete($objet->echeance_client($val, $objet->getDate($datedeb),$objet->getDate($datefin),$clientdebut,$clientfin,$type_reg,$facComptabilise,$typeTiers));
...
<table id="table" class="table table-striped table-bordered" cellspacing="0">
    ...
    <?php
        ...
        if($rows==null){
            ...
            echo "<td></td><td></td><td></td><td>{$objet->formatChiffre($totalttc)}</td><td>{$objet->formatChiffre($totalrc)}</td><td>{$objet->formatChiffre($totalreste)}</td></tr>";
        }
...
<table>
    <tr style='background-color: #46464be6;color: white;font-weight: bold;'>
        ...
        <td style="padding:10px"><?= $objet->formatChiffre($totalMontantG); ?></td>
        <td style="padding:10px">Montant avance : </td>
        <td style="padding:10px"><?= $objet->formatChiffre($totalMontantAvanceG); ?></td>
        <td style="padding:10px">Reste à payer : </td>
        <td style="padding:10px"><?= $objet->formatChiffre($totalResteAPayerG); ?></td>
    </tr>
</table>
<?php 
    }
