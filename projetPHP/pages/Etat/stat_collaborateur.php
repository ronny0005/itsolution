...
<form action="indexMVC.php?module=5&action=11" method="GET">
    ...
    <div class="form-group col-lg-3">
        ...
        <input type="submit"  class="btn btn-primary" value="Imprimer" <?php  echo "onClick=\"window.open('./export/exportStatCollabParClient.php?datedeb=".$datedeb."&datefin=".$datefin."&do_type=".$do_type."&depot=".$depot_no."&rupture=".$rupture."')\""; ?> />
    </div>
</form>
...
$result=$objet->db->requete($objet->depot());     
    ...
    foreach($rows as $row){
        ...
        $result=$objet->db->requete($etatList->stat_collaborateurClient($val, $objet->getDate($datedeb), $objet->getDate($datefin),$do_type));
        $rows = $result->fetchAll(PDO::FETCH_OBJ);
        $i=0;
        $qte=0; 
        $cumul=0;
    ...
    <tbody>
        ...
        if($rows==null){
            ...
            foreach ($rows as $row){
                if($ref!=$row->CO_Nom){
                if($cmpligne>0){
                    if($soustotalCAHT*100!=0)
                        $spourc=$soustotalMarge/$soustotalCAHT*100;
                    else
                        $spourc=0;
                   ...
                   if($flagPxRevient==0)echo"<td>".$objet->formatChiffre($soustotalMarge)."</td><td>".$objet->formatChiffre(ROUND($spourc,2))."</td>";
                    echo "</tr>";
                }
                    $soustotalCAHT=0;
                    $soustotalCATTC=0;
            ...
            if($cmpligne>0){
                $val1=0;
                if($soustotalCAHT!=0)
                $objet->formatChiffre(ROUND($soustotalMarge/$soustotalCAHT*100,2));
                echo "<tr style='background-color:#a4a4a4;font-weight: bold'><td>Sous total $ref</td><td></td><td>".$objet->formatChiffre($soustotalCAHT)."</td><td>".$objet->formatChiffre($soustotalCATTC)."</td><td></td>";
        ...
        "<td>".ROUND($MARGE/$CA_NET_HT*100,2)."</td></tr>";}
        }
