...
<form action="indexMVC.php?module=5&action=20" method="GET">
    ...
    <div class="form-group col-lg-3">
        ...
        <input type="submit"  class="btn btn-primary" value="Imprimer" <?php  echo "onClick=\"window.open('./export/exportStatArticleFournisseurParAgence.php?type=mvtStock&datedeb=".$datedeb."&datefin=".$datefin."&depot=".$depot_no."&rupture=".$rupture."&article=".$article."&famille=".$famille."&do_type=".$do_type."')\""; ?> />
    </div>
</form>
...
$result=$objet->db->requete($objet->depot());     
    ...
    foreach($rows as $row){
        ...
        $result=$objet->db->requete($objet->stat_articleFournisseurParAgence($val, $objet->getDate($datedeb), $objet->getDate($datefin),$famille,$article,$do_type));
        $rows = $result->fetchAll(PDO::FETCH_OBJ);
        $i=0;
        $canetht=0;
        $canetttc=0;
...
<table id="table" class="table table-striped table-bordered" cellspacing="0">
    ...
    <?php
        ...
        echo "<td>Total</td><td></td><td></td><td>".$objet->formatChiffre($canetht)."</td><td>".$objet->formatChiffre($precompte)."</td><td>".$objet->formatChiffre($canetttc)."</td>";
...
<table>
    <tr style='background-color: #46464be6;color: white;font-weight: bold;'>
        ...
        <td style="padding:10px">Quantités ache : </td>
