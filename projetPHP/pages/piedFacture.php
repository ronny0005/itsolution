...
<fieldset id="liste_reglement" class="entete" style="<?php if($isVisu == 0) echo "display:none"; ?>">
    ...
    <table class="table table-striped" id="tableRecouvrement">
                            ...
                            <tbody>
                                 <?php
                                    $result=$objet->db->requete($objet->getReglementByClientFacture($docEntete->DO_Tiers,$docEntete->DO_Piece,$docEntete->DO_Type,$docEntete->DO_Domaine));
...
<form action="indexMVC.php?action=3&module=2" method="GET" name="form-valider" id="form-valider">
    ...
    <div class="col-sm-2">
        <button type="button" class="btn btn-primary" id="annuler" disabled>Annuler</button>
    </div>
    <?php
    if ($isModif /*&& $reste_a_payer!=0*/){ ?>
    <div class="col-sm-2">
        <button type="button" class="btn btn-primary" id="valider" disabled>Valider</button>
     </div>
    <?php } ?>
    <div class="col-sm-2">
        <button type="button" class="btn btn-primary" id="imprimer" disabled>Imprimer</button>
     </div>
    </form>
</div>
<?php
