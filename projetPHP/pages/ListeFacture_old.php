...
function lienfinal($statut,$avance,$entete,$type,$depot,$admini,$cbMarq,$do_domaine,$do_type,$do_modif,$administrateur,$protectedDocP,$flagProtApresImpressionP,$do_imprim){
    ...
    if($type=="BonLivraison" ||$type == "Retour" ||$type == "Vente" ||$type == "VenteDevise" ||$type == "VenteC"||$type == "VenteT"||$type == "RetourC"||$type == "RetourT" ||$type == "AchatC" ||$type == "AchatT" || $type == "Achat"|| $type == "PreparationCommande"|| $type == "AchatPreparationCommande"){
        $complement="&modif=1";
        if($administrateur==1)
            if($statut=="comptant")
                $complement="&visu=1";
...
?>
                    ...
                    if(sizeof($listFacture)==0){
                        ...
                        foreach ($listFacture as $row){
                            ...
                            $avance="";
                            $total = round($row->ttc);
                            if($type=="Ticket" || $type=="BonLivraison" || $type=="Retour" || $type=="Vente" || $type=="VenteC" || $type=="VenteT" || $type=="RetourC" || $type=="RetourT" || $type=="Achat" || $type=="AchatC"  || $type=="AchatT" || $type=="PreparationCommande"|| $type=="AchatPreparationCommande"){
...
?>
                            ...
                            <?php
                            if($type=="Ticket" || $type=="BonLivraison" || $type=="Vente" || $type=="Retour" || $type=="AchatT" || $type=="VenteT"  || $type=="VenteC" || $type=="RetourT"  || $type=="RetourC" || $type=="Achat" || $type=="AchatC" || $type=="PreparationCommande"|| $type=="AchatPreparationCommande")
